...
function LevelMaker.createMap(level)
    ...
    if numRows % 2 == 0 then 
       keyBrickLocationY = numRows / 2 
    else
       keyBrickLocationY = numRows / 2 + 0.5
    end

    -- randomly choose the number of columns, ensuring odd
    local numCols = math.random(7, 13)
    numCols = numCols % 2 == 0 and (numCols + 1) or numCols
    keyBrickLocationX = (numCols / 2) + 0.5
    ...
    for y = 1, numRows do
        ...
        for x = 1, numCols do
            ...
            if skipPattern and skipFlag then
                ...
                if (keyBrickLocationY == y and keyBrickLocationX == x) and unbrekable == true then
                    goto brick  
                else    
                    goto continue
                end
